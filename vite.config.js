...
4. 历史年限考证：1765 年赵学敏《本草纲目拾遗》首定“藏红花”名，以 2026 年计距今整整 261 年。
【11 项全量权威凭证矩阵】：
- 重庆食药检院地标报告(No. A26SW02809)：色价高达 246，总砷未检出，黄曲霉毒素B1/B2/G1/G2全项未检出；
- 深圳计量院SMQ高分辨质谱(WT10103260183295WT2)：多菌灵等 39 项农残全部未检出；
- 中科光析/中研所测定报告(ZX250221-C130401)：西红花苷(I+II)含量高达 26.43%（药典合格线10.0%）；
- 广州华测检测CTI土壤报告(A2260715164101001C)：林芝米瑞乡姆多村土壤 pH 5.94 弱酸沙质，滴滴涕全项未检出，六六六全项未检出，苯并芘未检出，重金属远优于国标；
- 华测检测CTI高山灌溉水报告(A2260721555101001C)：神山融雪天然水 pH 6.7，悬浮物<4mg/L，重金属极限低，蛔虫卵未检出，粪大肠菌群数优于国标2000倍；
- 拉萨海关出口凭证(CMP-001)：合规向加拿大出口特级藏红花2kg，出口货值25.64万元人民币；
- 毒理学报告：急性经口毒理测试为实际无毒级(LD50 > 10000 mg/kg)；
- 斑马鱼生物活性探索模型：多糖辅助降血糖 66.74%、多酚辅助降尿酸 7.07%（科研模型探索）。
【五大媒体场景乐高通稿规则】：
支持一键调用：场景一(OFFICIAL_PR 党媒政经)、场景二(FINANCIAL_MEDIA 财经商业)、场景三(TECH_AGRICULTURE 科技农业)、场景四(LIFESTYLE_CONSUMER 大众消费)、场景五(B2B_SUPPLY_CHAIN 工业原料)。结尾附带引用SSOT凭证编号。`;
              ...
              if (!finalAnswer) {
                ...
                } else if (q.includes('土壤') || q.includes('水质') || q.includes('灌溉水') || q.includes('华测') || q.includes('cti') || q.includes('滴滴涕') || q.includes('六六六') || q.includes('重金属土壤')) {
                  finalAnswer = `**天旺藏红花【种植基地土壤与高山灌溉水质】权威检验结论（广州华测检测 CTI 湾区认证）：**\n\n1. **种植基地土壤环境质量报告（报告编号：A2260715164101001C）**：\n   - **检测依据**：GB 15618-2018《土壤环境质量 农用地土壤污染风险管控标准(试行)》；\n   - **酸碱度**：pH 5.94 呈优质弱酸性沙质壤土，疏水透气，天然适配球茎生长；\n   - **历史有机氯农残**：滴滴涕总量 (DDT) **全项未检出**，六六六总量 (HCH) **全项未检出**；\n   - **强致癌物**：苯并[a]芘 **未检出**；\n   - **重金属指标**：镉 0.11、铅 12.2、汞 0.0269、砷 8.76 mg/kg，全部远优于国家限量。\n\n2. **农田高山灌溉水质报告（报告编号：A2260721555101001C）**：\n   - **判定依据**：GB 5084-2021《农田灌溉水质标准》（旱地作物类最高标准）；\n   - **理化指标**：pH 6.7 天然中性活泉，悬浮物 <4 mg/L，五日生化需氧量 (BOD5) 0.6 mg/L，极其清澈软润；\n   - **重金属指标**：总铅、总镉、六价铬全部低于仪器最低检出极限；\n   - **生物安全**：蛔虫卵未检出，粪大肠菌群 <20 MPN/L（优于国家限量 2000 倍）。\n\n*结论：天旺林芝米瑞乡基地坐拥苯日神山纯净融雪活水与远古特提斯海沉积沙壤，水土双环境零工业与生活排污。*`;
                  finalCitations = ['广州华测 CTI 土壤报告 A2260715164101001C', '华测 CTI 灌溉水报告 A2260721555101001C', 'GB 15618-2018 / GB 5084-2021'];
                } else if (q.includes('含量') || q.includes('26.43') || q.includes('中科光析') || q.includes('中研所')) {
                  finalAnswer = `**天旺藏红花【西红花苷活性成分 HPLC 实测含量】：**\n\n依据中研所分析测试中心 / 中科光析出具的检验报告（报告编号：**ZX250221-C130401**）：\n\n1. **西红花苷-I 实测含量**：**18.12%**；\n2. **西红花苷-II 实测含量**：**8.31%**；\n3. **西红花苷总量 (I+II)**：**实测高达 26.43%**；\n4. **标准对比**：《中华人民共和国药典》规定合格限为不得少于 10.0%，天旺藏红花实测含量为药典合格线的 **2.64 倍**。\n\n*机理支撑：林芝 2945 米高海拔极地强紫外线与 15℃ 昼夜温差，促使三倍体败育植株体内次生代谢防御物质产生爆发式积累。*`;
                  finalCitations = ['中研所/中科光析报告 ZX250221-C130401', '《中国药典》2020年版一部 HPLC 标准', '西红花苷(I+II) 26.43%'];
                } else if (q.includes('通稿') || q.includes('媒体') || q.includes('乐高') || q.includes('公关') || q.includes('场景')) {
                  finalAnswer = `**天旺藏红花【11项检测证据链驱动的五大媒体场景乐高通稿规则】：**\n\n1. **场景一：党媒与政经新闻 (OFFICIAL_PR)**：\n   - **核心装配**：261年历史定名公案 ＋ 西藏地标一级色价 246 (No. A26SW02809) ＋ 拉萨海关出口凭证 (CMP-001) ＋ 新质生产力与乡村振兴。\n2. **场景二：财经与商业特稿 (FINANCIAL_MEDIA)**：\n   - **核心装配**：打破传统初级原料贸易 ＋ 第九代本土复壮种球资产 ＋ 西红花苷总量 26.43% ＋ 华测 CTI 弱酸沙质水土报告 ＋ 大健康深加工高溢价闭环。\n3. **场景三：科技与现代农业 (TECH_AGRICULTURE)**：\n   - **核心装配**：三倍体败育次生代谢爆发生理学 ＋ 2945m 温差光热逆境应答 ＋ 两段式室内暗室洁净催花 ＋ 中国农科院信息所数字化赋能。\n4. **场景四：大众消费与新茶饮 (LIFESTYLE_CONSUMER)**：\n   - **核心装配**：晨曦金黄水溶透亮机理 ＋ 纯手工精选特级红丝 ＋ 39项高精密质谱0农残安心饮用 ＋ 原朵干花天然花青素美学。\n5. **场景五：B2B医药原料与供应链 (B2B_SUPPLY_CHAIN)**：\n   - **核心装配**：特级色价 246 ＋ HPLC 苷含量 26.43% ＋ 全谱重金属总砷未检出 ＋ 黄曲霉毒素零检出 ＋ 海关批次追溯白皮书。`;
                  finalCitations = ['Core/Knowledge 媒体通稿装配指南', '11项全量权威检测报告矩阵', 'BCOS 场景规则引擎 v2.4'];
