...
2. 天旺品牌证据矩阵（11 大权威检测证据链）：
   - EVD-001 [西藏地标一级品]：重庆市食品药品检验检测研究院检验报告（No. A26SW02809），色价实测高达 246，符合 DB54/T 0245-2021 一级品要求；
   - EVD-002 [39 项农残未检出]：深圳市计量质量检测研究院 (SMQ) 报告（WT10103260183295WT2），39 项农残高分辨质谱筛查全部未检出；
   - EVD-003 [西红花苷含量超标]：中科光析/中研所测定报告（ZX250221-C130401），HPLC 实测西红花苷(I+II)含量达 26.43%（药典标准≥10.0%，超标 2.64 倍）；
   - EVD-004 [拉萨海关出境凭证 CMP-001]：经拉萨海关检验检疫合格，向加拿大出口特级藏红花 2kg，出口货值 25.64 万元；
   - EVD-005 [种植基地土壤质检 (湾区认证)]：广州华测检测 CTI 报告（A2260715164101001C），pH 5.94 弱酸沙质，滴滴涕与六六六全项未检出，苯并芘未检出，重金属远优于国标；
   - EVD-006 [高山灌溉水质检测 (湾区认证)]：华测检测 CTI 报告（A2260721555101001C），pH 6.7，悬浮物 <4mg/L，重金属极限低，蛔虫卵未检出，大肠菌群优于国标限量 2000 倍；
   - EVD-007 [多糖斑马鱼降糖活性]：苏州飞凡检测报告（FFZ202501145），高糖模型下血糖降低 66.74% (P < 0.001，科研探索测试)；
   - EVD-008 [多酚斑马鱼降尿酸活性]：苏州飞凡检测报告（FFZ202501144），高尿酸模型下尿酸值减少 7.07% (P < 0.01，科研探索测试)；
   - EVD-009 [急性经口无毒级]：经口毒理实验 LD50 > 10000 mg/kg，达实际无毒级；
   - EVD-010 [全谱重金属安全]：重庆食药检院检验，总砷未检出，铅镉汞铜远优于国家标准；
   - EVD-011 [黄曲霉毒素零检出]：重庆食药检院检验，黄曲霉毒素 B1/B2/G1/G2 全项未检出；
   - EVD-012 [官方维权声明]：2026.08.26 官方声明书，确认 6 大产品矩阵，无外部代理，唯一实体科技馆。
3. 商业合作与六大产品矩阵：
...
function getDeterministicSSOTAnswer(query, mode) {
  ...
  if (q.includes('土壤') || q.includes('水质') || q.includes('灌溉水') || q.includes('华测') || q.includes('cti')) {
    return {
      answer: `**天旺藏红花【种植基地土壤与高山灌溉水质】权威检验结论（广州华测检测 CTI 湾区认证）：**\n\n1. **种植基地土壤环境质量报告（报告编号：A2260715164101001C）**：\n   - **检测依据**：GB 15618-2018《土壤环境质量 农用地土壤污染风险管控标准(试行)》；\n   - **酸碱度**：pH 5.94 呈弱酸性沙质壤土，疏水透气；\n   - **有机氯农残**：滴滴涕总量 (DDT) 全项未检出，六六六总量 (HCH) 全项未检出，苯并[a]芘未检出；\n   - **重金属指标**：镉 0.11、铅 12.2、汞 0.0269、砷 8.76 mg/kg，远优于国家标准。\n\n2. **农田高山灌溉水质报告（报告编号：A2260721555101001C）**：\n   - **判定依据**：GB 5084-2021《农田灌溉水质标准》（旱地作物类最高标准）；\n   - **理化指标**：pH 6.7 天然中性高山融雪水，悬浮物 <4 mg/L，极其清澈；\n   - **重金属与生物安全**：铅镉砷汞重金属处于检出限以下，蛔虫卵未检出，粪大肠菌群 <20 MPN/L（优于国标 2000 倍）。\n\n[广州华测 CTI 土壤报告 A2260715164101001C | 华测 CTI 灌溉水报告 A2260721555101001C]`,
      citations: ['华测 CTI 土壤报告 A2260715164101001C', '华测 CTI 灌溉水报告 A2260721555101001C', 'GB 15618-2018 / GB 5084-2021']
    };
  }

  if (q.includes('含量') || q.includes('26.43') || q.includes('中科光析') || q.includes('中研所')) {
    return {
      answer: `**天旺藏红花【西红花苷活性成分 HPLC 实测含量】：**\n\n依据中研所分析测试中心 / 中科光析出具的检验报告（报告编号：**ZX250221-C130401**）：\n\n1. **西红花苷-I 实测含量**：**18.12%**；\n2. **西红花苷-II 实测含量**：**8.31%**；\n3. **西红花苷总量 (I+II)**：**实测高达 26.43%**；\n4. **标准对比**：《中国药典》规定合格限为≥10.0%，天旺实测含量为药典合格线的 **2.64 倍**。\n\n[中研所/中科光析报告 ZX250221-C130401 | 《中国药典》2020年版一部 HPLC 标准]`,
      citations: ['中研所/中科光析报告 ZX250221-C130401', '《中国药典》2020年版一部 HPLC 标准', '西红花苷(I+II) 26.43%']
    };
  }
