...
function refreshDrillControl() {
  ...
  const quickTibetBtn = currentState.scope === 'country'
    ? `<button type="button" data-map-action="quick-tibet" style="background:rgba(212,160,23,0.3);border-color:#f7d668;color:#fef08a;font-weight:600">🏔️ 直达西藏林芝</button>`
    : '';
  ...
  drillControlEl.innerHTML = `
    ...
    ${quickTibetBtn}
    <button type="button" data-map-action="save-view-default" ${actionDisabled}>保存视角</button>
    <button type="button" data-map-action="reset-view-all" ${actionDisabled}>恢复视角</button>
  `;
  drillControlEl.querySelectorAll('button').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      if (action === 'quick-tibet') {
        void drillToFeature('西藏自治区');
        return;
      }
