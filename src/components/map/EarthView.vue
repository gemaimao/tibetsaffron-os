...
<template>
  ...
  >
    ...
    <div class="earth-interactive-hud" aria-hidden="true">
      <span class="hud-pulse-ring" />
      <span class="hud-hud-tip">点击发光的中国版图下钻 3D 地图 · 拖拽自由探索地球</span>
    </div>
  </div>
</template>
...
function createInternationalFlyNetwork() {
  ...
  const routes = [
    // 藏红花全球起源与战略合作节点
    { lon: -3.7038, lat: 40.4168 }, // 西班牙 · 拉曼查 (Spain La Mancha 种球源头)
    { lon: 23.7275, lat: 37.9838 }, // 地中海 · 希腊雅典
    { lon: 51.3890, lat: 35.6892 }, // 伊朗 · 德黑兰
    { lon: 121.5300, lat: 31.6200 }, // 上海 · 崇明岛 (两段式平原大田繁育基地)
    { lon: 114.1694, lat: 22.3193 }, // 香港 (宝芝林全球研发战略伙伴)
    { lon: 114.9754, lat: 30.0988 }, // 湖北 · 黄石 (劲牌/劲酒产业伙伴)
    { lon: 104.0665, lat: 30.5728 }, // 成都 · 茶马古道集散口岸
    { lon: 116.4074, lat: 39.9042 }, // 北京 (首都)
  ];
  // 核心枢纽：西藏林芝米瑞乡天旺基地 (2945m)
  const hub = { lon: 94.5541, lat: 29.4763 };
...
.earth-interactive-hud {
  ...
  box-shadow: 0 4px 20px rgba(0, 0, 0, 0.6), inset 0 0 12px rgba(212, 160, 23, 0.15);
  backdrop-filter: blur(8px);
  pointer-events: none;
  animation: hud-float 3s ease-in-out infinite alternate;
}
...
@keyframes ring-pulse {
  ...
  50% { transform: scale(1.3); opacity: 1; box-shadow: 0 0 14px #e8c766; }
